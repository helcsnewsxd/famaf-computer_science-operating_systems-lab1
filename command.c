...
#include "command.h"
#include "strextra.h"
// Needed library for queue defined structure (GQueue)
...
unsigned int scommand_length(const scommand self){
    ...
    unsigned int sc_length = g_queue_get_length(self->sc_arguments);

    assert((sc_length == 0) == scommand_is_empty(self));
    return sc_length;
}
...
char * scommand_front(const scommand self){
    ...
    char * sc_front = g_queue_peek_head(self->sc_arguments);

    assert(sc_front != NULL);
    return sc_front;
}
...
char * scommand_to_string(const scommand self){
    ...
    if(sc_length != 0){
        ...
        for(unsigned int i = 1; i < sc_length; i++){
